...
## About page "What's next" + real-world Gemini validation
Added a "What's next" section to the site's About page, directly channeling the strategy reset above: names what's done and free forever (label format, registry, badges) versus what's early prototype (the Docs integration), and invites real users now rather than promising a finished detection system. Committed and pushed along with the Add-on source (`apps-script/plgen-connector/`) and both session log files, which had been sitting locally unpushed.

Separately, Shelton tested the concept directly with Gemini in a Google Doc ("PL Test 3"). Two findings: (1) asked directly, Gemini confirmed it "cannot directly execute network requests... from this chat interface" and recommended "the ideal architecture is for the extension to handle that network request" — independent confirmation, from Gemini itself, of the same architecture already chosen (Workspace Add-on over Gemini Gem). (2) Gemini has a "Personalization" saved-instructions feature (Settings > Personalization) that persisted a label-format instruction across a session without a custom Gem — a lighter-weight path for self-report formatting than building a Gem, not yet explored further.

## Gemini Personalization instruction drafted and refined
Built the actual instruction text for that Personalization field (1024-char limit). Caught and fixed a real staleness bug in the process: the existing instruction hardcoded `PL v1.0`, but the standard has been on v1.2 since 2026-06-01. First pass (745 chars) added the missing behavior — silent contribution tracking during the session, non-persuadable split estimation (carried over from the full `plgen.txt` prompt's integrity rule), and confirm-before-output — in readable prose. Second pass, at Shelton's request, restructured the same content as directive rule-labels (ROLE/TRACK/TRIGGER/numbered steps) instead of prose — 590 chars, same constraints, no content dropped, likely *more* reliable for instruction-following since procedural steps parse more deterministically than narrative sentences.

## Stale v1.0 references fixed sitewide
The Gemini-instruction staleness catch prompted a sweep for the same bug elsewhere. Found and fixed: the homepage's interactive format demo, all three "Examples" section entries, the spec page's live "what a label looks like" illustration, and the registration form's JSON placeholder — all still showed v1.0. Left two references alone deliberately: the About page's own self-label (a real label generated in Feb 2026 when v1.0 was actually current — changing it would misrepresent history) and the spec page's changelog table row documenting the v1.0 release itself. Committed and pushed.
